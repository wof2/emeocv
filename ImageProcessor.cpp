...
public:
    bool operator()(cv::Vec2f const & a, cv::Vec2f const & b) const {
        return  ImageProcessor::getLinePointAvgY(a) < ImageProcessor::getLinePointAvgY(b);
    }
};
...
float  ImageProcessor::getLinePointAvgY(cv::Vec2f line) {
        float rho = line[0];
        float theta = line[1];
        double a = cos(theta), b = sin(theta);
        double x0 = a * rho, y0 = b * rho;
...
std::vector<cv::Rect> ImageProcessor::findAlignedBoxesFromCounterArea(cv::Mat edges) {
	log4cpp::Category& rlog = log4cpp::Category::getRoot();
    ...
    if (_debugEdges) {
        // draw contours
        cv::Mat contb = cv::Mat::zeros(edges.rows, edges.cols, CV_8UC1);
